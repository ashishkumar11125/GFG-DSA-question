...
class Solution {
public:
    // Function to check if the binary search tree has a dead end.
    bool check(Node* root, int l, int h) {
        // If the current node is null, return false.
        if (!root) return false;

        // If the range [l, h] becomes a single number, return true (dead end).
        if (l == h) return true;
    ...
    bool isDeadEnd(Node* root) {
        // code here
        // Start the initial check with the entire range of integers (1 to INT_MAX).
